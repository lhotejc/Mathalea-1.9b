...
import {liste_de_question_to_contenu_sans_numero,randint,reduire_ax_plus_b,tex_nombre,SVG_Tracer_droite,Latex_Tracer_droite,SVG_repere,Latex_repere,katex_Popup2} from "/modules/outils.js"
	...
	this.nouvelle_version = function (numero_de_l_exercice) {
		...
		for (let i = 0; i < nb_droites; i++) {
			...
			if (this.lineaire || liste_droites[i][0] == 0)
				this.liste_corrections.push(`La droite $d_${i + 1}$ passe par l'origine et son coefficient directeur est $${tex_nombre(liste_droites[i][1])}$.<br>Elle représente la fonction linéaire $f_${i + 1}(x)=${reduire_ax_plus_b(liste_droites[i][1], 0)}$.`);
			else
				this.liste_corrections.push(`La droite $d_${i + 1}$ passe par le point de coordonnées $(0;${liste_droites[i][0]})$ et son coefficient directeur est $${tex_nombre(liste_droites[i][1])}$.<br>Elle représente la fonction affine $f_${i + 1}(x)=${reduire_ax_plus_b(liste_droites[i][1], liste_droites[i][0])}$.`);
