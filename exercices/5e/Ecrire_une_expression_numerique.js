...
export default function Ecrire_une_expression_numerique() {
	...
	this.nouvelle_version = function () {
		...
		for (let i = 0, texte, texte_corr, val1, val2, cpt = 0; i < this.nb_questions && cpt < 50;) {
			...
			val1 = randint(2, 5);
			val2 = randint(6, 9);
			if (this.version > 2 && nb_operations == 1 && !this.litteral)
				nb_operations++;
			if (!this.litteral)
			...
			switch (this.version) {
				...
				case 3:
					...
					if (!this.litteral)
						...
						texte = `${expf} puis calculer pour $x=${val1}$.`;
					texte_corr = `${expf} s'écrit ${expn}.<br>`;
					if (!this.litteral)
						texte_corr = `${expc}.`;
					else if (nbval == 2)
						texte_corr += `Pour $x=${val1}$ et $y=${val2}$ :<br> ${expc}.`;
					else
						texte_corr += `Pour $x=${val1}$ :<br>${expc}.`;
					break;
				case 4:
					if (expn.indexOf('ou') > 0)
						expn = expn.substring(0, expn.indexOf('ou')); // on supprime la deuxième expression fractionnaire
