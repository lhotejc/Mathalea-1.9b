import {estentier,randint,arrondi,simplification_de_fraction_avec_etapes,calcul,tex_nombrec,tex_nombre,nombre_avec_espace,tex_fraction} from "/modules/outils.js"
/**
			...
			switch (souscas) {
				...
				case 3: // quotient de deux nombres
					...
					if (estentier(val1 / b * 1000))
						expc = `$${l1}\\div ${tex_nombre(b)} = ${val1}\\div ${tex_nombre(b)} = ${tex_nombrec(val1 / b)}$`;
					else
						expc = `$${l1}\\div ${tex_nombre(b)} = ${val1}\\div ${tex_nombre(b)}=${tex_fraction(val1, tex_nombre(b))}${simplification_de_fraction_avec_etapes(val1, tex_nombre(b))}$`;
					last_op = 'division';
					break;
			}
			break;
